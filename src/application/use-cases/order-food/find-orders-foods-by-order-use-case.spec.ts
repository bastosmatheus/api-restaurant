...
import { FindOrdersFoodsByOrderUseCase } from "./find-orders-foods-by-order-use-case";
...
let findOrdersFoodsByOrderUseCase: FindOrdersFoodsByOrderUseCase;
let bcryptAdapter: BcryptAdapter;

describe("get order food by order", () => {
  beforeEach(() => {
    ...
    findOrdersFoodsByOrderUseCase = new FindOrdersFoodsByOrderUseCase(orderFoodRepository);
  });

  it("should be possible to get order food by order", async () => {
    ...
    const orderFood = await findOrdersFoodsByOrderUseCase.execute({ id_order });
