...
import { FindOrdersByCardUseCase } from "./find-orders-by-card-use-case";
...
let findOrdersByCardUseCase: FindOrdersByCardUseCase;
let bcryptAdapter: BcryptAdapter;

describe("get orders by card", () => {
  beforeEach(() => {
    ...
    findOrdersByCardUseCase = new FindOrdersByCardUseCase(orderRepository);
  });

  it("should be possible to get orders by card", async () => {
    ...
    const orders = await findOrdersByCardUseCase.execute({ id_card });
